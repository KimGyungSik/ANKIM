...
class MailApiControllerTest {
    ...
    public void sendMailTest() throws Exception {
        ...
        when(mailService.createMail(loginId, code)).thenReturn(mimeMessage);
    ...
    public void test2() throws Exception {
        ...
        String code = "123456";
        when(mailService.generateCode()).thenReturn(code);
        MailRequest mailRequest = new MailRequest(loginId, code);

        when(mailService.verifyCode(loginId, code)).thenReturn(Count.SUCCESS); // 성공 케이스로 설정
        ...
        // then
        verify(mailService, times(1)).verifyCode(loginId, code);
    }
    ...
    public void resendCodeUpdatesCode() throws Exception {
        ...
        when(mailService.createMail(loginId, firstCode)).thenReturn(mimeMessage);
        ...
        when(mailService.createMail(loginId, secondCode)).thenReturn(mimeMessage);
    ...
    public void resendAfterThreeFailuresThenSuccess() throws Exception {
        ...
        when(mailService.createMail(loginId, initialCode)).thenReturn(mimeMessage);
        ...
        String wrongCode = "W0NgCd";
        when(mailService.verifyCode(loginId, wrongCode)).thenReturn(Count.FAIL);

        // when: 인증번호 3회 틀리기
        for (int i = 0; i < 3; i++) {
            mockMvc.perform(post("/api/mail/verify")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(new MailRequest(loginId, wrongCode))))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.message").value("FAIL"));
        }

        // given: 3회 실패 후 "RETRY" 설정
        when(mailService.verifyCode(loginId, wrongCode)).thenReturn(Count.RETRY);
        ...
        when(mailService.createMail(loginId, newCode)).thenReturn(mimeMessage);
        ...
        when(mailService.verifyCode(loginId, newCode)).thenReturn(Count.SUCCESS);

        // when: 새로운 인증번호로 성공적인 검증 요청
        mockMvc.perform(post("/api/mail/verify")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new MailRequest(loginId, newCode))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("SUCCESS"));
    }
