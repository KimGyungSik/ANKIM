...
class PaymentFacadeWithNamedLockTest {
    @MockBean
...
//    }
    ...
    """, order.getOrdNo());
        ...
        // when
        PaymentSuccessResponse response = paymentFacadeWithNamedLock.toSuccessRequest(paymentKey, order.getOrdNo(), amount);
...
//
//        // when
//        PaymentFailResponse failResponse = paymentFacadeWithNamedLock.toFailRequest(errorCode, errorMessage, orderId);
//
//        // then
...
//
//        // 재고 복구 검증
//        Item item1 = order.getOrderItems().get(2).getItem();
//        Item item2 = order.getOrderItems().get(3).getItem();
//
//        // EntityManager를 통해 최신 상태 조회
...
//
//        // when
//        PaymentCancelResponse response = paymentFacadeWithNamedLock.toCancelRequest(paymentKey, cancelReason);
//
//        // then
