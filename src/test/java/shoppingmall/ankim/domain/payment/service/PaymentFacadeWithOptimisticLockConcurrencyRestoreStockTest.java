...
public class PaymentFacadeWithOptimisticLockConcurrencyRestoreStockTest {
    @MockBean
    ...
    @Autowired
    private PaymentFacadeWithOptimisticLock paymentFacadeWithOptimisticLock;
    ...
    void tossPaymentFail() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    paymentFacadeWithOptimisticLock.toFailRequestWithOptimisticLock(code, message, orderId);
                } catch (Exception e) {
    ...
    void cancelPayment() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    paymentFacadeWithOptimisticLock.toCancelRequestWithOptimisticLock(paymentKey, cancelReason);
                } catch (Exception e) {
                    failureCount.incrementAndGet(); // 실패 시 카운트 증가
                    e.printStackTrace();
                } finally {
