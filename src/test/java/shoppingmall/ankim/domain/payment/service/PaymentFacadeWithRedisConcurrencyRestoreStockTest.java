...
public class PaymentFacadeWithRedisConcurrencyRestoreStockTest {
    @MockBean
...
//                    orderRepository.save(order);
    ...
    void tossPaymentFail() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    paymentFacadeWithRedis.toFailRequestWithRedis(code, message, orderId);
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
                    latch.countDown();
    ...
    void cancelPayment() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    paymentFacadeWithRedis.toCancelRequestWithRedis(paymentKey, cancelReason);
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
                    latch.countDown();
