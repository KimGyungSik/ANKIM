        ...
        mockMvc.perform(get("/api/cart")
                ...
                .andDo(document("cart-get-items",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("message").description("응답 메시지").type(JsonFieldType.STRING),
                                fieldWithPath("fieldErrors").description("필드 오류 목록").optional().type(JsonFieldType.ARRAY),
                                fieldWithPath("data").description("응답 데이터 (장바구니 품목 목록)").type(JsonFieldType.ARRAY).optional(),
                                fieldWithPath("data[].cartNo").description("장바구니 번호").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].cartItemNo").description("장바구니 품목 번호").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].itemNo").description("품목 번호").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].productName").description("상품명").type(JsonFieldType.STRING),
                                fieldWithPath("data[].itemName").description("품목명").type(JsonFieldType.STRING),
                                fieldWithPath("data[].thumbNailImgUrl").description("썸네일 이미지 주소").type(JsonFieldType.STRING),
                                fieldWithPath("data[].qty").description("주문 수량").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].itemQty").description("품목 재고량").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].sellingStatus").description("판매 상태").optional().type(JsonFieldType.STRING),
                                fieldWithPath("data[].origPrice").description("정상가격").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].discRate").description("할인율").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].sellPrice").description("판매가격").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].addPrice").description("추가금액 (품목에 대한 추가금)").type(JsonFieldType.NUMBER),
                                fieldWithPath("data[].totalPrice").description("판매가 (할인율 적용된 가격) + 추가금액").type(JsonFieldType.NUMBER),
